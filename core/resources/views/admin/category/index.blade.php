                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="button--group">
                                                <button type="button" class="btn btn-sm btn-outline--primary ms-1 editBtn" data-action="{{ route('admin.category.save', $category->id) }}" data-name="{{ $category->name }}" data-description="{{ $category->description }}"
                                                ...
                                                <div class="dropdown-menu more-dropdown">
                                                    @if ($category->status == Status::DISABLE)
                                                        <button class="dropdown-item confirmationBtn" data-action="{{ route('admin.category.status', $category->id) }}" data-question="@lang('Are you sure to enable this category?')">
                                                            <i class="la la-eye"></i> @lang('Enable')
                                                        </button>
                                                    @else
                                                        <button class="dropdown-item confirmationBtn" data-action="{{ route('admin.category.status', $category->id) }}" data-question="@lang('Are you sure to disable this category?')">
                                                            <i class="la la-eye-slash"></i> @lang('Disable')
                                                        </button>
                                                    @endif

                                                    @if ($category->featured)
                                                        <button class="dropdown-item confirmationBtn" data-action="{{ route('admin.category.feature', $category->id) }}" data-question="@lang('Are you sure to unfeature this category?')">
                                                            <i class="las la-times"></i> @lang('Make Unfeatured')
                                                        </button>
                                                    @else
                                                        <button class="dropdown-item confirmationBtn" data-action="{{ route('admin.category.feature', $category->id) }}" data-question="@lang('Are you sure to feature this category?')">
                                                            <i class="las la-check"></i> @lang('Make Featured')
                                                        </button>
                                                    @endif
                                                    @if ($category->show_on_banner)
                                                        <button class="dropdown-item confirmationBtn" data-action="{{ route('admin.category.banner', $category->id) }}" data-question="@lang('Are you sure to hide this category from banner?')">
                                                            <i class="las la-eye-slash"></i> @lang('Hide from banner')
                                                        </button>
                                                    @else
                                                        <button class="dropdown-item confirmationBtn" data-action="{{ route('admin.category.banner', $category->id) }}" data-question="@lang('Are you sure to show this category on banner?')">
