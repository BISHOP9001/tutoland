                            ...
                            <tbody>
                                @forelse ($lessons as $lesson)
                                    <tr>
                                        ...
                                        @if (!$sectionLesson)
                                            <td>
                                                <a href="{{ route('admin.course.section', $lesson->section->id) }}">{{ __(strLimit($lesson->section->title)) }}</a>
                                            </td>
                                        @endif
                                        <td>
                                            @if ($lesson->server)
                                        ...
                                        <td>
                                            <div class="button--group">
                                                <button class="btn btn-sm btn-outline--primary editBtn" data-lesson="{{ $lesson }}" data-price="{{ getAmount($lesson->price) }}" data-image="{{ getImage(getFilePath('video_thumb') . '/' . $lesson->thumb_image) }}">
                                                ...
                                                <div class="dropdown-menu more-dropdown">
                                                    <a href="{{ route('admin.course.lesson.video', $lesson->id) }}" class="dropdown-item">
                                                    ...
                                                    @if ($lesson->status == Status::DISABLE)
                                                        <button class="dropdown-item confirmationBtn" data-action="{{ route('admin.course.lesson.status', $lesson->id) }}" data-question="@lang('Are you sure to enable this video?')">
                                                            <i class="la la-eye"></i> @lang('Enable')
                                                        </button>
                                                    @else
                                                        <button class="dropdown-item confirmationBtn" data-action="{{ route('admin.course.lesson.status', $lesson->id) }}" data-question="@lang('Are you sure to disable this video?')">
                                                            <i class="la la-eye-slash"></i> @lang('Disable')
                                                        </button>
                                                    @endif
                                                    <button class="dropdown-item assetFileBtn" data-lesson_id="{{ $lesson->id }}" data-asset_file="{{ asset(getFilePath('lesson_asset')) }}">
                                                        <i class="las la-book"></i> @lang('Asset File')
                                                    </button>
                                                    @if ($lesson->asset_path)
                                                        <a href="{{ route('admin.course.lesson.asset.download', $lesson->id) }}" class="dropdown-item" data-lesson_id="{{ $lesson->id }}" data-asset_file="{{ asset(getFilePath('lesson_asset')) }}">
                        ...
                        <div class="row">
                            <div class="col-lg-5">
                                <div class="form-group">
                                    <label class="imageLabel">@lang('Thumb Image')</label>
                                    <div class="image-upload">
                                        <div class="thumb">
                                            <div class="avatar-preview">
                                                <div class="profilePicPreview" style="background-image: url({{ getImage(getFilePath('course') . '/', getFileSize('course')) }})">
                                                    <button type="button" class="remove-image"><i class="fa fa-times"></i></button>
                                                </div>
                                            </div>
                                            <div class="avatar-edit">
                                                <input type="file" class="profilePicUpload" name="thumb_image" id="profilePicUpload1" accept=".png, .jpg, .jpeg" required>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="section_id" class="form-control" required>
                                        ...
                                        @foreach ($course->sections as $section)
                                            <option value="{{ $section->id }}" @selected(isset($sectionId) && $sectionId == $section->id)>{{ __($section->title) }}</option>
                                        @endforeach
                                    </select>
                                </div>
    ...
    <div id="lessonAssetModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="" method="POST" id="assetForm" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="form-group">
                        ...
                        <div class="form-group progressGroup">
                            ...
                            <div class="progress">
                                <div class="progress-bar progress-bar-striped progress-bar-animated customWidth" role="progressbar" aria-valuenow="75" aria-valuemin="0" aria-valuemax="100"></div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
