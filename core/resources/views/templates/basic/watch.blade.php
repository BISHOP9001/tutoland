            ...
            <div class="row gy-4 gy-lg-0 gx-lg-4">
                <div class="col-lg-8">
                    <div class="course-view-inner course-view-inner-left">
                        <video class="video-js course-view-video" id="your-video" data-setup="{}" controls preload="auto" poster="{{ getImage(getFilePath('video_thumb') . '/' . $currentLesson->thumb_image) }}" controlsList="nodownload">
                ...
                <div class="col-lg-4">
                        ...
                        <div class="accordion" id="course-content-accordion">
                            ...
                            <div class="accordion-item">
                                <div class="accordion-header">
                                    <button class="accordion-button @if ($section->id != $currentLesson->section_id) collapsed @endif" data-bs-toggle="collapse" data-bs-target="#course-content-accordion-item-{{ $loop->iteration }}" type="button" aria-expanded="{{ $section->id == $currentLesson->section_id ? 'true' : 'false' }}" aria-controls="course-content-accordion-item-{{ $loop->iteration }}">
                                        <div class="accordion-headings">
                                            <h5 class="heading">@lang('Section - '){{ $loop->iteration }}: {{ __($section->title) }}</h5>
                                            <span class="sub-heading">{{ secondsToHMS($section->lessons->sum('video_duration')) }}</span>
                                        </div>
                                    </button>
                                </div>

                                <div class="accordion-collapse @if ($section->id == $currentLesson->section_id) show @endif collapse" id="course-content-accordion-item-{{ $loop->iteration }}" data-bs-parent="#course-content-accordion">
                                        ...
                                        <ul class="course-lesson-content">
                                            ...
                                            <li class="course-lesson-content-item">
                                                <a class="course-lesson-content-video-link" href="{{ $lessonUrl }}">
                                                    ...
                                                    @else
                                                    <span class="icon scroll-here"><i class="fas fa-check-circle"></i></span>
                                                    @endif
                                                    <span class="text">{{ $loop->iteration }}. {{ __($lesson->title) }}</span>
                    ...
                    <div class="form-group" >
                        <button id="certificationButton" class="btn btn--base w-100" type="button">@lang('Get my certification')</button>
                    </div>
                        ...
                        <form action="{{ route('user.course.review') }}" method="POST">
                            ...
                            <div class="form-group review-form-group col-md-6 d-flex mb-20 mt-3 flex-wrap">
                                ...
                                <div class="rating-form-group">
                                    @for ($i = 1; $i <= 5; $i++) <label class="star-label">
                                        <input name="rating" type="radio" value="{{ $i }}" {{ $userReview && $userReview->rating == $i ? 'checked' : '' }}>
                                        @for ($j = 1; $j <= $i; $j++) <span class="icon"><i class="las la-star"></i></span>
                                            @endfor
                                            </label>
                                            @endfor
                                </div>
...
var videoElement = document.getElementById('your-video');
var progressInterval;

if (!videoElement) {
    console.error('Video element not found for lesson ID ' + lessonId);
}

// Initialize video resume time based on user's progress
var resumeTime = '{{ $userLessonProgress->pause_time }}';
if (resumeTime) {
    videoElement.currentTime = resumeTime;
}

videoElement.addEventListener('play', function() {
    // Additional logic if needed when the video starts playing
});
...
function sendProgressData() {
    var seconds = Math.floor(videoElement.currentTime);
    var status = (videoElement.duration * 0.8 < seconds) ? 'completed' : 'pending';
    ...
    if (seconds >= 12) {
        ...
        $.ajax({
            method: 'POST',
            url: '{{ route("update.lesson.progress") }}',
            data: data,
...
// Periodically send progress every 30 seconds
