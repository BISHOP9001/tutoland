            ...
            <div class="row g-4">
                @foreach ($premiumCourses as $pCourse)
                    <div class="col-lg-4 col-sm-6">
                        <a class="card course-card h-100" href="{{ route('course.details', [slug($pCourse->title), $pCourse->id]) }}">
                            <div class="card-header">
                                <img class="course-card-thumb" src="{{ getImage(getFilePath('course') . '/' . $pCourse->image) }}" alt="{{ __($pCourse->title) }}">
                                <span class="course-card-badge">@lang('Premium')</span>
                            </div>
                            <div class="card-body">
                                <h6 class="course-card-title">{{ __($pCourse->title) }}</h6>
                                <div class="flex-between gap-2">
                                    <span class="course-card-duration">@lang('Duration:') {{ secondsToHMS($pCourse->total_duration) }}</span>
                            ...
                            <div class="card-footer">
                                <span class="course-card-price fs-16">
                                    @if ($pCourse->discount_price > 0)
                                        <del class="text-muted fs-15">{{ $general->cur_sym }}{{ showAmount($pCourse->price) }}</del>
                                        {{ $general->cur_sym }}{{ showAmount($pCourse->discount_price) }}
                                    @else
                                        {{ $general->cur_sym }}{{ showAmount($pCourse->price) }}
                                    @endif
                                </span>
        ...
        <div class="py-60">
            ...
            <div class="row g-4">
                @foreach ($freeCourses as $fCourse)
                    <div class="col-lg-4 col-sm-6">
                        <a class="card course-card h-100 price-free" href="{{ route('course.details', [slug($fCourse->title), $fCourse->id]) }}">
                            <div class="card-header">
                                <img class="course-card-thumb" src="{{ getImage(getFilePath('course') . '/' . $fCourse->image) }}" alt="{{ __($fCourse->title) }}">
                            </div>
                            <div class="card-body">
                            ...
                            <div class="card-footer">
                                <span class="course-card-price">{{ $general->cur_sym }} @lang('Free')</span>
                                <button class="btn btn--rounded btn--base-two course-card-btn-enroll" type="button">
                                    <i class="fas fa-play-circle"></i>
                                    @lang('Enroll now')
                                </button>
                            </div>
