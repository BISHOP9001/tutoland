        ...
        <section class="@if ($layout == 'frontend') py-120 @endif">
            <div class="custom--container">
                <div class="dashboard-body">
                    <div class="card @if($layout == 'frontend') custom--card @endif">
                                            ...
                                            @if ($message->attachments->count() > 0)
                                                <div class="mt-2">
                                                    @foreach ($message->attachments as $k => $image)
                                                        <a href="{{ route('ticket.download', encrypt($image->id)) }}" class="me-3 text--primary"><i class="fa fa-file"></i> @lang('Attachment') {{ ++$k }} </a>
                                                    @endforeach
                                                </div>
                                            @endif
                                        </div>
                                    ...
                                    <div class="row border border-warning border-radius-3 my-3 py-3 mx-2" style="background-color: #ffd96729">
                                        ...
                                        <div class="col-md-9">
                                            ...
                                            @if ($message->attachments->count() > 0)
                                                <div class="mt-2">
                                                    @foreach ($message->attachments as $k => $image)
                                                        <a href="{{ route('ticket.download', encrypt($image->id)) }}" class="text--primary me-3"><i class="fa fa-file"></i> @lang('Attachment') {{ ++$k }} </a>
                                                    @endforeach
                                                </div>
                                            @endif
                                        </div>
            ...
            $('.addFile').on('click', function() {
                ...
                $("#fileUploadsContainer").append(`
                    <div class="input-group my-3">
                        ...
                        <button type="submit" class="input-group-text btn--danger text-white remove-btn border-0"><i class="las la-times"></i></button>
                    </div>
                `)
            });
