...
class CourseController extends Controller
{
    public function list()
    {
        ...
        }])->where('user_id', auth()->id())->where('status', CoursePurchase::APPROVED)->orderBy('id', 'desc')->paginate(getPaginate());

        $myReviews = Review::where('user_id', auth()->id())->pluck('course_id')->toArray();

        return view($this->activeTemplate . 'user.course.list', compact('pageTitle', 'myCourses', 'myReviews'));
    }
