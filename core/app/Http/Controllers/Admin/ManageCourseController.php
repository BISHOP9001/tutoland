...
class ManageCourseController extends Controller
{
    ...
    {
        $course    = Course::findOrFail($id);
        $pageTitle = 'Section of - ' . $course->title;
    ...
    public function lessonsBySection($sectionId)
    {
        ...
        $pageTitle = 'Lessons of - ' . $section->title;
    ...
    {
        $pageTitle = "Course QCM";
        $quiz = Quiz::with('course', 'questions')->where('course_id', $courseId)->first();
        return view('admin.course.quiz', compact('pageTitle', 'quiz', 'courseId'));
    }
    public function createcourseQuiz($courseId)
    {
        $pageTitle = "Add Course QCM";
        return view('admin.course.quiz_add', compact('pageTitle', 'courseId'));
    }
    public function storeQuiz(Request $request)
    {
        try {
            ...
            $quiz = Quiz::with('course')->where('course_id', $courseId)->first();
            if (!$quiz) {
                $quiz = new Quiz(
                    [
                        "course_id" => $courseId,
            ...
            return response()->json(['status' => 'success', 'question' => $newquestion]);
        } catch (\Exception $e) {
